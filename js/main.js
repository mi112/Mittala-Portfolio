!function(ze) {
    "use strict";
    function Ge(e, n, r) {
        for (var t, i = document.getElementById(e), o = i.innerHTML.replace("&amp;", "&").split(""), a = "", u = 0, f = o.length; u < f; u++)
            a += (t = o[u].replace("&", "&amp")).trim() ? '<span class="letter-' + u + '">' + t + "</span>" : "&nbsp;";
        i.innerHTML = a;
        setTimeout(function() {
            i.className = "transition-in " + (r || ""),
            "function" == typeof n && setTimeout(n, 200 * o.length)
        }, 500 * Math.random() + 500)
    }
    ...
    ze.onload = function() {
        ...
        function e() {
            ...
            function n(e) {
                (i = t.createBufferSource()).connect(t.destination),
                i.buffer = e,
                i.onended = function() {
                    i.loop = !1
                }
            ...
            ,
            r.open("GET", "audio/es_intrepid_dye_o.mp3"),
        ...
        ze.addEventListener("click", function() {
            t || (t = new (ze.AudioContext || ze.webkitAudioContext),
            e(),
        ...
        setInterval(function() {
            c.push(parseInt(10 * Math.random()) + 3)
        }, 2e4);
        var m = function(e) {
            var n, r, t = {
                alpha: !0,
        ...
        function() {
            ...
            e.add(l, "DYE_RESOLUTION", {
                high: 1024,
                medium: 512,
                low: 256,
                "very low": 128
            }).name("quality").onFinishChange(Ee),
            e.add(l, "SIM_RESOLUTION", {
                32: 32,
                64: 64,
                128: 128,
                256: 256
            }).name("sim resolution").onFinishChange(Ee),
            e.add(l, "DENSITY_DISSIPATION", 0, 4).name("density diffusion"),
            e.add(l, "VELOCITY_DISSIPATION", 0, 4).name("velocity diffusion"),
            e.add(l, "PRESSURE", 0, 1).name("pressure"),
            e.add(l, "CURL", 0, 50).name("vorticity").step(1),
            e.add(l, "SPLAT_RADIUS", .01, 1).name("splat radius"),
            e.add(l, "SHADING").name("shading").onFinishChange(Se),
            e.add(l, "COLORFUL").name("colorful"),
            e.add(l, "PAUSED").name("paused").listen(),
            e.add({
                fun: function() {
                    c.push(parseInt(20 * Math.random()) + 5)
                }
            }, "fun").name("Random splats");
            var n = e.addFolder("Bloom");
            n.add(l, "BLOOM").name("enabled").onFinishChange(Se),
            n.add(l, "BLOOM_INTENSITY", .1, 2).name("intensity"),
            n.add(l, "BLOOM_THRESHOLD", 0, 1).name("threshold");
            var r = e.addFolder("Sunrays");
            r.add(l, "SUNRAYS").name("enabled").onFinishChange(Se),
            r.add(l, "SUNRAYS_WEIGHT", .3, 1).name("weight"),
            T() && e.close()
        }();
        var g = (a(E, [{
            key: "setKeywords",
